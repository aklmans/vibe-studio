...
- **Lecture · left / right** (1920×1080) — a lecture frame: a header band (brand logo + series name, plus today's topic and a date + LIVE badge once the session starts), a presenter column (camera above an intro card with the stream title, presenter name and affiliation lines), and an exact-16:9 slides region. Each section can carry an optional **speaker** — the presenter card introduces the active section's guest (with a quiet hosted-by line), falling back to the host.
...
Any layout can flip into **fullscreen main focus**: a runtime toggle (Overlay inspector → Display, the agenda drive console, or ⌘K) that lets the main capture take over the entire canvas — the bottom bar, sidebar, camera and every other panel disappear, while the thin main frame and its corner accent stay so the room remains recognizable. It's a temporary zoom of the same scene, not a fifth layout: the other panels' own visibility toggles, agendas and bars are untouched, and exiting restores the standard frame exactly as it was. The OBS composition controls follow the toggle — with a composition applied, entering fullscreen re-parks the main capture full-bleed and disables every camera source, exiting restores the standard regions (the camera picker pauses with a note while fullscreen is on), and the browser-source overlay mirrors the state live.
