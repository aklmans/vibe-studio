...
The agenda holds **up to 12 sections**, and bullets are optional — a pure agenda item is just a title plus planned minutes. The shared sections manager (Overlay inspector and **Session Config → Session**) adds, removes and reorders sections and their bullets; every structural change atomically keeps the active section, per-bullet done flags and each bar profile's progress segment pointing at the right section. Selecting a section there drives the agenda (and stamps the section timer) too. On the broadcast canvas the sidebar shows a **sliding window of 3 sections** starting at the active one (pulled back at the tail so the window stays full), with a small `0X–0Y / 0Z` indicator once there are more than 3 — with 3 or fewer, it renders exactly as before.
