...
底栏在其段落仍是未改动默认组合时会跟随布局:工作台用 开播 / 进度 / 栈,讲座布局用 开播 / **议程** / **关注** —— 议程段直接从你的章节渲染"当前第 n/N 节 + 下一节",在 studio 里推进讲座,直播画面的议程条自动更新;关注段让第一个可见社交账号常驻画面。手动改过的底栏永远不会被动;两个新段落类型在任何布局的底栏编辑器里都可选用。
