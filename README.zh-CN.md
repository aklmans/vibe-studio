...
任意布局都可切入**全屏主画面**:一个运行时开关(覆盖层 inspector → 显示、议程驾驶台、或 ⌘K),让主捕获吃满整张画布 —— 底栏、侧栏、摄像头等所有其他面板全部隐藏,细边框与角标保留,画面依然认得出是这间直播间。它是同一场景的临时放大,不是第五种布局:各面板自己的显示开关、议程与底栏都不被触碰,退出即原样恢复标准画面。OBS 画面合成同样跟随这个开关 —— 在已应用合成的情况下,进入全屏会把主捕获重新铺满整张画布并停用所有摄像头源,退出即恢复标准区域(全屏期间相机位选择会暂停并给出提示),浏览器源覆盖层则实时镜像该状态。
