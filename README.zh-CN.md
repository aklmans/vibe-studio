...
- **手机竖屏**(1080×1920)—— 自上而下拼接的竖版框架:header、屏幕分享(主区域)、其下的摄像头、纤细底栏、底部讲者卡。两个区域都是 OBS 填充的挖洞,合成控件可用 —— 把 OBS 基准画布设为 1080×1920(竖屏 profile)即可对上槽位坐标;也可导出为图片框架供手机 App 推流使用。
...
讲座布局中,讲者卡在讲者信息之下带有**流程清单**:编号行 + 计划分钟,当前节之前的节标记为完成(强调色勾 + 划线),当前节带强调色 rail 与实时"已用 / 计划"计时,后续节保持安静。超过 5 节时与侧栏一样滑窗,并带同款 mono 指示。
...
Schema 在 `src/db/schema.ts`,SQL 迁移已提交在 `drizzle/` 下(至 `0005_agenda_profiles.sql` —— 场景独立议程)。数据库 API 挂在 `/api/sessions` 下,而 `/api/live-state` 仍是实时 OBS 桥接。
