...
- **手机竖屏**(1080×1920)—— 面向手机 App / 直播姬竖屏直播的竖版框架:header、一整块由手机自身摄像头或画面从底下填充的视频挖洞、底部讲者卡。它没有本机 OBS 合成 —— 可导出为图片框架或作为浏览器源使用;推流留在手机 App。

底栏在其段落仍是未改动默认组合时会跟随布局:工作台用 开播 / 进度 / 栈,讲座布局用 开播 / **议程** / **关注**。议程段直接从你的章节渲染"当前第 n/N 节 + 本节已用时 / 计划时长 + 下一节" —— 每节可选填计划时长(分钟,属 v1 内容,Agent 起草时可一并安排),每次切节都会重计本节时间;关注段展示你指定的社交账号(未指定则回退到第一个可见)。手动改过的底栏永远不会被动;两个段落类型在任何布局的底栏编辑器里都可选用。
