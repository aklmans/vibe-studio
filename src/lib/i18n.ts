...
export const dict = {
  zh: {
    ...
    "agentPrepare.tasksLabel": "快捷任务",
    "agentPrepare.step1": "复制提示词，在你自己的 AI 工具里运行。",
    "agentPrepare.step2": "打开 JSON 抽屉，导入返回的 JSON。",
    "agentPrepare.step3": "检查无误后 Apply 应用（导入不会自动应用）。",
    ...
    "configView.settings": "设置",
    "drawer.openJson": "打开 JSON",
    "drawer.close": "关闭",
    "drawer.jumpTo": "跳转到",
    "settingsView.hint": "手动调整本场配置与运行状态。可移植核心会进入 live-session.config.json，运行时控制不会。",
    "settingsView.brandNote": "标题、封面、徽标、社交、头像在 Cover / Poster 面板编辑；它们都是核心，会出现在 JSON 里。",
    "settingsView.runtimeNote": "当前段落与完成状态也是运行时——在上方 Sections 里切换与勾选。",
    "settingsView.displayNote": "侧栏可见性与 surface 映射是运行时显示——在 Overlay 面板里切换。",
    "settingsView.openStudio": "打开工作室设置",
    "settingsView.appearanceNote": "主题、颜色、重置在工作室设置里（顶栏 ⚙ 也可进入）。",
    "settingsView.persistenceNote": "数据库是持久化 / 历史，不是配置源；OBS live-state 是单向镜像，不在此反向编辑。",
  ...
  en: {
    ...
    "agentPrepare.tasksLabel": "Tasks",
    "agentPrepare.step1": "Copy the prompt and run it in your own AI tool.",
    "agentPrepare.step2": "Open the JSON drawer and import the returned JSON.",
    "agentPrepare.step3": "Review, then Apply (import never auto-applies).",
    ...
    "settingsView.hint": "Manually adjust this stream's config and runtime. The portable core goes into live-session.config.json; runtime controls do not.",
    "settingsView.brandNote": "Title, cover, badges, socials and avatar are edited in the Cover / Poster inspector; they are all core and appear in the JSON.",
    "settingsView.runtimeNote": "Active section and done states are runtime too — set them in Sections above.",
    "settingsView.displayNote": "Sidebar visibility and surface mapping are runtime display — toggle them in the Overlay inspector.",
    "settingsView.openStudio": "Open studio settings",
    "settingsView.appearanceNote": "Theme, colors and reset live in studio settings (also the ⚙ in the top bar).",
    "settingsView.persistenceNote": "DB is persistence / history, not the config source; OBS live-state is a one-way mirror, not editable here.",
    "settingsGroup.core": "Session Core",
