...
export function buildChatMessages(request: SessionAgentRequest): ChatMessage[] {
  ...
  const system = [
    ...
    "You edit ONLY the per-stream content of live-session.config.json (v1): version, title, subtitle, badges: string[], stack: string[], sections: [{ title, minutes?, speaker?, bullets?: string[] }] (up to 12 sections). minutes is the section's optional planned duration in whole minutes (the on-air agenda timer uses it); bullets are optional — a pure agenda item is just a title + minutes; speaker is the optional per-section presenter/guest name (only when the brief names one).",
